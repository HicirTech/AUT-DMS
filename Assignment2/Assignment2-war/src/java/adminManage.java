...
//   private HttpSession userSession = request.getSession();
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<h1>Welcome back admin "+this.cu.getUserName()+", you login time is "+cu.getLoginTime()+"</h1>");
            out.println("what would you want to do?");
            out.println("<form method=\"GET\"'>"
                 + "<button name='' type='submit'>review a user</button>"
                 + "</form>");
