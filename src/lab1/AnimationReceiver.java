...
public class AnimationReceiver extends JPanel
{
   ...
   {  setPreferredSize(new Dimension(PANEL_WIDTH,PANEL_HEIGHT));
      ...
      animatedImage = new BufferedImage(PANEL_WIDTH, PANEL_HEIGHT,
   ...
   public void receive()
   {  try
      {  System.out.println("Receiver started at "
      ...
      DatagramPacket receiveDatagram = new DatagramPacket(buffer,
         buffer.length);
      while (!stopRequested)
      {  try
         {  socket.receive(receiveDatagram);
            ...
            synchronized(this)
            {  animatedImage.setRGB(x, y, blockSize,
                  blockSize, pixels, 0, blockSize);
            }
         }
         catch (IOException e)
